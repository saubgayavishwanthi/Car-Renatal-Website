...
import PeroduaAxia from "../assets/cartypes/Perodua Axia.jpg";
...
function RentCar() {
  ...
  const [openBasicInfo, setOpenBasicInfo] = useState(null); // Track which card is open

  const toggleBasicInfo = (id) => {
    setOpenBasicInfo(openBasicInfo === id ? null : id);
  };
      ...
      <div className="car-list">
        {filteredCars.map((car) => (
          <div key={car.id} className="car-card">
            ...
            <Tabs defaultActiveKey="rates" className="car-tabs">
              {/* Basic Info Tab - Click to Toggle */}
              <Tab
                eventKey="basicInfo"
                title={<span onClick={() => toggleBasicInfo(car.id)}>Basic Info</span>}
              >
                {openBasicInfo === car.id && (
